...
internal class Installer(private val context: Context, private var activity: Activity?) {
    ...
    fun installPackage(apkPaths: Array<String>, silent: Boolean = true) {
        try {
            session = createSession(silent)
    ...
    private fun createSession(silent: Boolean): PackageInstaller.Session {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                // Silence is opt-in: MIUI/HyperOS aborts USER_ACTION_NOT_REQUIRED
                // sessions from non-privileged installers without any dialog,
                // so foreground installs must request user action explicitly.
                if (silent) {
