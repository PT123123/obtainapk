        ...
        if (intent.action == packageInstalledAction && intent.extras != null) {
            ...
            when (val status = extras.getInt(PackageInstaller.EXTRA_STATUS)) {
                PackageInstaller.STATUS_PENDING_USER_ACTION -> {
                    if (activity == null) {
                        // No activity to launch the confirmation dialog (e.g.
                        // background isolate): fail instead of hanging forever.
                        MethodCallHandler.resultSuccess(PackageInstaller.STATUS_FAILURE_ABORTED)
                    } else {
